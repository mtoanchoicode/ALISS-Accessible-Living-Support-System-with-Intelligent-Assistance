...
self.addEventListener("message", async (e: MessageEvent) => {
  ...
  if (type === "INIT") {
    try {
      if (!model) {
        await tf.ready();
        model = await tf.loadGraphModel(self.location.origin + "/model_v2/model.json");
      }
  ...
  if (type === "DETECT" && model) {
    ...
    try {
      // 1. Pre-process and Run Inference wrapped in tf.tidy
      ...
      // 5. Scale boxes back
      const widthRatio = payload.videoWidth / 640;
      const heightRatio = payload.videoHeight / 640;

      // 6. Map to DetectedObject array
      const objects = (indices as number[])
        .map((idx) => {
      ...
      // Must explicitly close bitmap to prevent memory leak since it was transferred
      bitmap.close();
