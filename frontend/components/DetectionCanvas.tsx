...
export default function DetectionCanvas({
  ...
  const handleInteraction = (clientX: number, clientY: number) => {
    ...
    const clicked = [...objects]
      .reverse()
      .find((obj) => isPointInBox(x, y, obj.bbox));
    if (clicked) {
  ...
  return (
    <canvas
      ref={canvasRef}
      width={videoWidth} // The AI resolution (e.g., 1080)
      height={videoHeight} // The AI resolution (e.g., 1920)
      style={{
        width: "100%",
        height: "100%",
        objectFit: "contain", // CRITICAL: Must match the Webcam component
        position: "absolute",
        top: 0,
        left: 0,
      }}
      className={`z-10 touch-none ${className}`}
      onClick={handleClick}
